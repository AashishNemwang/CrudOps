...
export const createUser = (req, res) => {
    ...
    db.query(sql, email, (err, result) => {
        if (err) {
            ...
            if(result.length>0){
                res.send({statusCode:300, message:"Email is already registered!!"})
            }else{
...
export const updateUsers = (req, res) => {
    const { name, email, phone, password, hobby, address } = req.body;
    const {id} = req.params; 
    const sql = "UPDATE user SET name=?, email=?, phone=?, password=?, hobby=?, address=? WHERE userId=?";
    const values = [name, email, phone, password, hobby, address, id]; 
